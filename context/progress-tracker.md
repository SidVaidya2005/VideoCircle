...
**Last completed:** 23 Connection quality and recovery — 227 unit (up 11), 114 e2e (up 4), lint, typecheck all green. Two bugs fixed that predate the feature: every disconnect pushed Home, so a dropped call was indistinguishable from pressing Leave, and `CallStatus` checked sharing before reconnecting, so a reconnect *during* a share was invisible. Both regression tests were seen to fail before being trusted. The measure-first call paid off again — LiveKit already preserves mic mute across a reconnect, so the restoration code the build plan assumed was never written. The reason→copy switch shipped missing four `DisconnectReason` members that `typecheck` passed cleanly; the unit test caught it, and a `never` guard now makes the compiler catch the next one
**Next:** 24 Error and edge states, then the Phase 6 checkpoint. Six device- or deploy-dependent F22 checks block F25, alongside the LiveKit webhook configuration that leaves F20 unproven until then
