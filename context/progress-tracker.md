...
**Last completed:** 22 Mobile pass — 216 unit, 110 e2e (up 6), lint, typecheck, production build all green. The sweep measured six surfaces × eight viewports and found one real bug: `/history` overflowed 181px at 768 and 209px in phone landscape, because `truncate` does nothing on an inline box and the cell is `block` above `sm:` — invisible to ten specs that only ever measured 360px. The call passed at every width including 740×360, so no landscape treatment was written. Two structural fixes found by reading rather than measuring: the missing `viewport` export that had left every safe-area rule inert since F09, and the sheet's own insets that `viewport-fit=cover` then made necessary. Home's bundle went 343 → 279 kB gzipped by deferring the Supabase browser client behind the sign-in click; still over the 200 kB target, recorded in `constraints.md` with what the remainder is
**Next:** 23 Connection quality and recovery. Six device- or deploy-dependent F22 checks now block F25, alongside the LiveKit webhook configuration that leaves F20 unproven until then
