...
**Last completed:** 20 Participation recording — the signed webhook endpoint and the participation writes behind it. All gates green: 198 unit tests, 96 e2e (9 new), lint, typecheck, and a clean production build. Idempotency comes from the partial unique index rather than a dedupe ledger, and every timestamp comes from LiveKit's event clock rather than `now()`. Building it surfaced a latent Phase 0 bug: the nightly sweep's `left_at = expires_at` violates the row's own CHECK for anyone who joined after expiry, which aborts the sweep for *every* meeting — reproduced against the live schema, fixed with a `greatest()` clamp, and confirmed to fail before the fix
**Next:** 21 Call history page — the first reader of what F20 writes. Note F20 is unproven against real LiveKit traffic until the dashboard is configured at F25
