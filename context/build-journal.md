...
### F22 Mobile pass — 2026-08-14

**Decisions.**

- Everything needing a phone or a deployed URL was carried to F25 rather than faked. A phone cannot use camera or microphone against a local server — no secure context — and a tunnel would prove media on a real device against an origin that is still not the deployed instance. Six checks moved; none was dropped.
- The audit is one new spec (`responsive.spec.ts`) over new shared helpers (`support/viewport.ts`), and the ten existing per-feature 360px blocks were left where they are. Each is its own feature's regression guard, and consolidating them touches every call spec at once — work F26 already carries alongside the duplicated `joinAs`.
- The sweep asserts **softly**. It exists to produce a list, and a hard assertion turns thirty-plus measurements into one finding per run. Safe only because nothing downstream reads a measurement.
- Landscape was measured before being treated, and needed no treatment: the call passed both measurements at 740×360, and the three controls that may never collapse were asserted in-viewport there. No short-viewport layout was written, because nothing asked for one.
- The Home bundle was bounded work — measure, take local wins, record the rest — exactly as `code-standards.md` → Performance prescribes for a target that cannot be met.

**Gotchas.**

- **The sweep's one real find was invisible at the width the whole suite had been testing.** `/history` overflowed by 181px at 768 and 209px in phone landscape while passing at 360, 390, 430, 1024, 1440 and 1920. `truncate` sets `overflow: hidden`, which inline boxes ignore; below `sm:` the cell is flex and blockifies the span, above it the cell is `block` and the span stays inline, so the nowrap name list became the grid column's min-content. Fixed with a flex row whose names truncate and whose `+N more` is `shrink-0` — the count being eaten by the ellipsis explaining it would be worse than no truncation.
- **Reading found two bugs the sweep never could.** `src/app/layout.tsx` exported no `viewport`, so `viewport-fit=cover` was never set and every `env(safe-area-inset-*)` in the project resolved to zero — the call's inset padding had been dead code since F09, and a desktop browser reports it as correct either way. Setting `cover` then created the second: content now extends *under* the cutouts, so `.call-surface` and `.sheet-surface` had to pay all four insets back. Half of that decision alone makes a notched phone worse than neither half.
- **`zod` cannot be removed from Home by tidying a component.** `start-meeting.tsx` looked like the culprit, but `src/lib/env.ts` parses with Zod at import and is on Home regardless — that parse is the deliberate fail-at-boot mechanism, not an oversight. The real win was elsewhere: the Supabase browser client, statically imported by `sign-in.ts` and so pulled onto Home for every signed-out visitor. Behind a dynamic `import()` it is 64 kB gzipped lighter.
- **That win touched the one path with no automated coverage at all.** Nothing in the suite had ever clicked Sign in. `sign-in.spec.ts` now intercepts Supabase's `/authorize` and asserts the provider, the `NEXT_PUBLIC_SITE_URL`-resolved redirect, and — the invariant — that the chat key appears nowhere in the outgoing URL while surviving in `sessionStorage`.
- **Aborting an intercepted navigation is not free.** `route.abort()` sends Chrome to its own error page, which left `sessionStorage` on an opaque origin and raced a later `goto` under parallel load — green solo, red in the full suite. Fulfilling the top-level navigation with `204` leaves the document exactly where it was.

**Verified.** Both halves of the sweep were seen to fail before being trusted: the overflow check by the two real `/history` failures it found, the hit-area check by temporarily raising the floor to 100px and watching it report 11 controls per viewport. `expectHitAreas` also asserts it measured something, so an empty selector cannot read as a clean sweep. `sign-in.spec.ts` was confirmed red with the `signInWithGoogle` call severed. `viewport-fit=cover` was confirmed by grepping the meta tag out of a production server's HTML rather than the source that emits it. Home's bundle was re-measured from a production `next start`: 343 → 279 kB gzipped. Gates: `lint` (3 pre-existing `call-preview.tsx` warnings, unchanged), `typecheck`, `build` clean; 216 unit and 110 e2e pass.

### F23 Connection quality and recovery — 2026-08-14

**Decisions.**

- A deliberate leave and a dropped call are told apart by `DisconnectReason`, not by a flag of our own. `CLIENT_INITIATED` still goes Home; everything else renders the notice, and the reason shapes the copy.
- The quality marker is degraded-only and colourless. This is where `architecture.md`'s red invariant and `code-standards.md`'s colour discipline actually collided — `signal` *is* `red-1`, so a red quality dot would have been a third meaning for red across up to twelve tiles. The invariant won and the colour-discipline line was corrected.
- The reconnect banner stays inside `CallStatus`: one component, one source of truth about the connection. A second overlay could disagree with the first and would cover faces at the moment you are squinting at them.
- Rejoin reloads rather than re-entering the lobby in state, because `stopPreview()` bumps the media hook's acquisition generations and its acquiring effect runs on mount — the lobby would render with a permanently dead preview. A reload keeps the `#k=` fragment, re-runs the server-side meeting lookup, and avoids adding a restart route through the most intricate hook in the project.
- Mic restoration was measured before being written, and then not written.

**Gotchas.**

- **The feature's two real bugs both predate it.** Every disconnect pushed Home, so a call that dropped left you on the landing page with no way to tell whether you had left or been dropped, and no way back but finding the link again. And `CallStatus` checked sharing before reconnecting, so a reconnect *during* a share was invisible — the state you already knew about hid the one you needed. Neither was in the build plan; both fell out of reading the code the plan pointed at.
- **`typecheck` passed a switch that was missing four enum members.** `DisconnectReason` has seventeen, not the thirteen a truncated read of the type file showed, and `noImplicitReturns` is off — so `SIP_TRUNK_FAILURE`, `CONNECTION_TIMEOUT`, `MEDIA_FAILURE` and `AGENT_ERROR` returned `undefined` and would have rendered a blank panel to someone whose call had just died. The unit test caught it on its first run. A `default` branch assigning to `never` now makes the compiler catch the next one, verified by deleting a case and watching the build name it.
- **The published docs undercount two enums.** Context7 lists four `ConnectionQuality` values and omits `Lost` — the one a recovery feature exists for. Installed types are the authority; a docs page is a summary.
- **LiveKit already preserves mic mute across a reconnect.** The build plan asserted it did not. Mute, drop, restore, and the control is still pressed. Writing the restoration anyway would have been a reapply racing the SDK's own restore, visible only on a real flaky network. The test stays, so a future SDK change surfaces here rather than in someone's call.
- **A recorded constraint caught nothing because it was not applied.** `constraints.md` → Testing already says an unscoped `getByRole('alert')` matches Next's route announcer; the terminal-disconnect test used one anyway, passed instantly against markup that is not ours, and left the real assertion racing an offline toggle. Reading the constraints is not the same as using them.
- **A terminal disconnect is reachable in about a minute** by outlasting LiveKit's retry budget with the context offline. Slow for one test, but it needs no test-only path through production code, and it covers the headline claim rather than conceding it.

**Verified.** All three regression tests were seen to fail before being trusted: the terminal notice with `onDisconnected` reverted to always pushing Home, the share banner with the old sharing-first precedence, and the exhaustiveness guard by deleting a case. Gates: `lint` (3 pre-existing `call-preview.tsx` warnings, unchanged), `typecheck` clean; 227 unit and 114 e2e pass.

### F24 Error and edge states — 2026-08-14

**Decisions.**

- No toast. Every transient failure already has a treatment on the surface it happens on, and a toast layer would give each a second home free to drift from the first. `architecture.md`'s listing of a `toast` primitive was stale anticipation, never built, and is corrected — the same shape as F23's colour line.
- Media-failure copy moved out of `media-state-notice.tsx` into a pure module, because three of its five states are unreachable from the suite and only a unit test can check them.
- The leak check is one shared pattern set used by both the unit tests and the API e2e, so the two halves cannot drift into disagreeing about what a leak is. It carries its own guard-the-guard test, for the same reason `chat.spec.ts` asserts its recording is non-empty.
- The share control is gated on `Connected` for *starting* only. Stopping stays available in every state including a reconnect, which is exactly when someone wants their screen to stop being broadcast.

**Gotchas.**

- **The feature's headline bullet was built, measured, and deleted.** Route-level `loading.tsx` is the conventional answer and was actively harmful on both routes: Next flushes the loading shell immediately, which sends the HTTP status, so `notFound()` afterwards can only swap the UI and `redirect()` degrades to a client-side navigation. A dead meeting link went 404 → 200, and signed-out `/history` went 307 → a 200 that rendered "Every meeting you joined" to someone not allowed to see it, stranding a no-JS visitor on a skeleton forever. Two existing tests caught the first; **nothing caught the second**, because the auth e2e only checks that you end up Home and the client-side redirect does eventually get you there. Both files removed, the mechanism recorded.
- **A full-suite run caught a muted mic coming back unmuted after a reconnect — correcting F23.** F23 measured mute preservation, saw it hold, and declined to write restoration code on that evidence. It has since passed 17 consecutive runs including 12 under four-way parallel load, so the rate is low and load-dependent; the failure is also silent and privacy-adjacent. F23's constraint was rewritten and an open follow-up carries the evidence. The lesson is not that measure-first was wrong — it is that a clean sample is not a proof, and the test that made the claim is what later disproved it.
- **The empty-room screen-share unpublish did not reproduce at all.** 32 solo publishes — 8 sequential, 24 under three-way parallel load, 8 more through the old camera-sourced picker stub — every one stayed up. `livekit-client` has not moved since F07, so it is not an SDK bump. Downgraded from known-bad to unproven. No regression test was added on purpose: if the behaviour is real but rare, that test becomes the flake F13 was avoiding by not writing it.
- **Next 16 wants `retry()`, not `reset()`.** Written from memory this would have been `reset()`, which only re-renders what it already had and recovers nothing when the fetch was the failure. Proved by making a page throw on its first render only and watching `retry()` land on the real page.
- **Verifying that no detail leaks needed visible text, not `textContent()`.** The first attempt failed because `body.textContent()` sweeps up the RSC flight payload in `<script>` tags, and Next deliberately embeds the real message, digest and stack there in development. `innerText` on `main` is the assertion that means what it says.
- **`global-error.tsx` brings its own everything** — `globals.css` and the typeface, because Next applies neither — and keeps a plain `<a>` with the lint rule disabled on that line: the root layout is what failed, and a soft navigation would re-render straight back into it.

- **My own share-gate fixture was wrong in a way only load revealed.** Holding the room in Connecting by pointing it at `wss://127.0.0.1:1` works until it does not: a refused address makes LiveKit report `JOIN_FAILURE` immediately, and the terminal disconnect notice built earlier in this same feature correctly replaces the room tree, control bar included. The test had been winning that race. A blackholed TEST-NET-1 address hangs instead of refusing, which is the state actually wanted. Its `route.fetch()` also needed the same one-shot retry `createMeeting` carries, for the same documented `next dev` connection reset.
- **The suite run also recurred the F08 camera-track flake — two live video tracks where one was expected — and this time the artifact was preserved** at `scratchpad/f24-suite-artifacts/`, which two follow-ups had asked for and nobody had done. It is the same duplicate-track shape as the original, so the purity fix made at F08 was not the whole cause.

**Verified.** The share gate and jump-to-latest were each seen to fail before being trusted. The error boundary was proved with a temporary throwing page, including that `retry()` genuinely recovers and that no message, digest or stack reaches the screen; the probe was removed rather than left as a test-only path. The leak check has its own test proving it still catches a leak. Gates: `lint` (3 pre-existing `call-preview.tsx` warnings, unchanged), `typecheck` clean; 230 unit and 124 e2e pass.
