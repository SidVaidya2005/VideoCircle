...
- **Not verified, and blocked:** the actual round trip through Google, the surviving session across reload and tab close, sign-out clearing it, the signed-in dropdown, and the trigger's `coalesce` against Google's real payload. All need the OAuth provider enabled — see `progress-tracker.md` → Follow-ups.
